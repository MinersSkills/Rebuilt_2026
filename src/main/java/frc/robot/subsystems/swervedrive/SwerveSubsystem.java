...
import swervelib.telemetry.SwerveDriveTelemetry.TelemetryVerbosity;
  ...
  public void alignSwerveWithAngle(DoubleSupplier rightX, DoubleSupplier leftY, double blueAngle){
    Pose2d pose = getPose();
    var alliance = DriverStation.getAlliance();

    double blueSide = Units.degreesToRadians(blueAngle);

    if (alliance.isPresent()){
      if (alliance.get() == DriverStation.Alliance.Blue){
        blueSide = swerveDrive.swerveController.headingCalculate(pose.getRotation().getRadians(), Units.degreesToRadians(blueAngle));

        ChassisSpeeds blueSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(rightX.getAsDouble(), 
                                                                        leftY.getAsDouble(),
                                                                        blueSide, 
                                                                        pose.getRotation()
                                                                    );

        swerveDrive.drive(blueSpeeds);
      } 

      else {

      double redSide = MathUtil.angleModulus(blueSide);
        
      ChassisSpeeds redSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(rightX.getAsDouble(), 
                                                                      leftY.getAsDouble(),
                                                                      redSide, 
                                                                      pose.getRotation()
                                                                    );

        swerveDrive.drive(redSpeeds);
      }
    }
  }

    public boolean isAtHeading(double blueAngleDeg) {
      Pose2d pose = getPose();

      double targetRad = Units.degreesToRadians(blueAngleDeg);

      var alliance = DriverStation.getAlliance();
      if (alliance.isPresent() && alliance.get() == Alliance.Red) {
          targetRad = Math.PI - targetRad;
      }

      double error = MathUtil.angleModulus(
          targetRad - pose.getRotation().getRadians()
      );
